...
def getToken(wskey):
    headers = {'cookie': wskey, 'User-Agent': ua, 'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
               'charset': 'UTF-8', 'accept-encoding': 'br,gzip,deflate'}
...
def get_sign():
    ...
    for i in range(3):
        try:
            ...
            res = '''
            {"clientVersion": "10.1.2", "client": "android", "sv": "120", "st": "1630903502023", "uuid": "b49f93d10b2e2503", "sign": "09932eb478b178c578204e70c8ee6ce5"}
            '''
...
def cloud_info():
    ...
    for i in range(3):
        try:
            # res = requests.get(url=url, verify=False, timeout=20).text
            res = '''
            {"code": 200,"update": 903,"jdurl": "https://me-api.jd.com/user_new/info/GetJDUserInfoUnion","User-Agent": "jdapp;android;10.1.2;11;0393465333165363-5333430323261366;network/wifi;model/M2102K1C;osVer/30;appBuild/89743;partner/lc023;eufv/1;jdSupportDarkMode/0;Mozilla/5.0 (Linux; Android 11; M2102K1C Build/RKQ1.201112.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/77.0.3865.120 MQQBrowser/6.2 TBS/045714 Mobile Safari/537.36"}
            '''
        except requests.exceptions.ConnectTimeout:
            logger.info("\n获取云端参数超时, 正在重试!" + str(i))
        except requests.exceptions.ReadTimeout:
